...
License plate OCR training code based on the GPLPR model proposed by Liu et al. in the paper *"Irregular License Plate Recognition via Global Information Integration."*
...
## Testing
To test the model, ensure that the config file specifies the path to the .pth file (e.g., [config file](config/test.yaml)), as shown in the example below:

```yaml
model_ocr:
  name: GPLPR
  load: <GPLPR_CHECKPOINT_PATH>
  args:
    nc: 3
    isSeqModel: True
    head: 2
...
```
python3 test_ocr.py --config ./config/test.yaml --save True --tag example
```

## Training from Scratch

To train the model from scratch, update the following variables in the [config file](config/train.yaml):
...
```
python3 train.py --config ./config/train.yaml --save True
```

Optionally, you can add the --tag argument, for versioning:
```
python3 train.py --config ./config/train.yaml --save True --tag example
```
...
```
For reference, you can check split files such as [split_all_rodosol.txt](split_all_rodosol.txt), which demonstrates this format. Do not commit local image folders or plate-label annotation files.
...
Here’s an example of the modified `config/train.yaml` file:
