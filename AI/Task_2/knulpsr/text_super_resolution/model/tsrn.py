...
class TPInterpreter(nn.Module):
    ...
    def forward(self, image_feature, tp_input, stroke_map):
        ...
        b, heads, l, hw = s.shape
        s=s.permute(0,2,1,3).reshape(b,l,-1)
        s = self.activation(self.fc_in2(s))
        N, L, C = s.shape

        s_pos = self.pe(torch.zeros((N, L, C)).to(device)).permute(1, 0, 2)
        s_mask = torch.zeros((N, L)).to(device).bool()
        s = s.permute(1, 0, 2)#l,b,h (26,b,1024)

        # print('sequence shape', x.shape)

        text_prior, pr_weights, stroke_prior, _ = self.transformer(x, x_mask, self.init_factor.weight, x_pos,
                                                  s, s_mask, s_pos,
                                                  tgt=x_im, spatial_size=(H, W)) # self.init_factor.weight
        text_prior = text_prior.mean(0)
        stroke_prior=stroke_prior.mean(0)
        ...
        stroke_prior = stroke_prior.permute(1, 2, 0).view(N, C, H, W)

        return torch.cat([text_prior,stroke_prior],1), pr_weights
...
class RecurrentResidualBlockTL(nn.Module):
    def __init__(self, channels, text_channels):
        ...
        self.gru1 = GruBlock(channels + text_channels+text_channels, channels)
        # self.prelu = nn.ReLU()
        self.prelu = mish()
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(channels)
