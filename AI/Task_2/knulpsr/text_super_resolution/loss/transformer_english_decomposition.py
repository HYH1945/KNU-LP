...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, attention_map=None):
        ...
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)#b,l,c -> b 16 l c/16
...
class Transformer(nn.Module):
    ...
    def forward(self, image, text_length, text_input, test=False, attention_map=None):
        ...
        # print('word attention map shape', word_attention_map.shape)
...
if __name__ == '__main__':
    image = torch.Tensor(32,1,32,128).cuda()#b,c,h,w
    model = Encoder().cuda()
    output = model(image)
    print(output.shape)
    ...
    text_input = torch.Tensor(32,10,1024).cuda()#b,c,l?
    image_input = torch.Tensor(32,1024,8,32).cuda()#b,c,h,w
