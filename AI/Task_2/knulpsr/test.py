...
class RecognizeCallback(Callback):
    ...
    def recognize(self, pl_module, batch, batch_idx, split="train"):
        ...
        #                      cv2.COLOR_RGB2BGR)
        # cv2.imwrite('outputs/latent.jpg'
        #             , cvimg)
        # exit(0)
    ...
    def eval(self, batch, images_sr, aster, aster_info, test_model_type):
        ...
        images_lr = self._to_bchw_or_fused_lr(images_lr)
        images_hr = self._to_bchw_or_fused_lr(images_hr)
        ...
        for batch_i in range(images_lr.shape[0]):
            ...
            fig = plt.figure()
            plt.title(label)
            lr_vis = np.clip(images_lr[batch_i, :3, :, :].cpu().numpy().transpose(1, 2, 0), 0.0, 1.0)
            hr_vis = np.clip(images_hr[batch_i, :3, :, :].cpu().numpy().transpose(1, 2, 0), 0.0, 1.0)
            sr_vis = np.clip(images_sr[batch_i, :3, :, :].cpu().numpy().transpose(1, 2, 0), 0.0, 1.0)
            plt.subplot(1, 3, 1)
            plt.imshow(lr_vis)
            ...
            if str_filt(predict_result_sr[batch_i], filter_mode) == str_filt(label, filter_mode):
                ...
                cvimg = cv2.cvtColor((sr_vis * 255).astype(np.uint8),
                                     cv2.COLOR_RGB2BGR)
