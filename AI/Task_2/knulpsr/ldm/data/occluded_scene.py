...
if __name__ == '__main__':
    # occ=occluded_lmdbDataset(['<OCCLUDED_PAIR_ROOT>'])
    # sample = occ.__getitem__(2)
    # img = sample['image']
    # occ_img=sample['OC_image']
    # img.save('outputs/ori.jpg')
    # occ_img.save('outputs/occ.jpg')
    # exit(0)

    random.seed(0)
    all = []
    labelmap = {}
    STR = lmdbDataset(roots=['<OCR_BENCHMARK_ROOT>'])
    print(len(STR))
    ...
    raw_dataset = []
    cnt = 0
    for path in ['<OCCLUDED_SOURCE_ROOT_A>', '<OCCLUDED_SOURCE_ROOT_B>']:
                    ...
                    if cnt == 1:
                        ...
                        for i in range(len(labelmap[label])):
                            ...
                            all[labelmap[label][i]][0].save(
                                f'outputs/tmp/fig{i}_{all[labelmap[label][i]][1]}.jpg')
                    if cnt <= 5:
                        img.save(f'outputs/tmp/occ{cnt}.jpg')
                        all[simid][0].save(f'outputs/tmp/ori{cnt}.jpg')

    print(cnt)

    random.shuffle(raw_dataset)
    tot = len(raw_dataset)
    select_lmdb_data(raw_dataset[:tot - tot // 10], '<OCCLUDED_TRAIN_ROOT>', tot - tot // 10,
                     min_text_len=1)
    select_lmdb_data(raw_dataset[tot - tot // 10:], '<OCCLUDED_TEST_ROOT>', tot // 10, min_text_len=1)
