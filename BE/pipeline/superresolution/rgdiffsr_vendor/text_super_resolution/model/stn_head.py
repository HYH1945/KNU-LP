...
class STNHead(nn.Module):
  def __init__(self, in_planes, num_ctrlpoints, activation='none', input_size=(16, 64), fc_in_features=512):
    ...
    self.fc_in_features = int(fc_in_features)
  ...
  def forward(self, x):
    ...
    # print("x:", x.shape)
    if x.size(1) != self.fc_in_features:
      raise RuntimeError(
        f"STNHead fc input mismatch: got {x.size(1)}, expected {self.fc_in_features}. "
        f"Check input_size/fc_in_features setting."
      )
