...
def get_high_resolution_count(candidates: list[SelectedCandidate], options: PipelineOptions) -> int:
    ...
    threshold = get_sr_area_threshold(options)
    return sum(
...
def get_sr_area_threshold(options: PipelineOptions) -> int:
    """Return the HR 80% area threshold used for automatic SR routing."""

    return int(options.hr_width * options.hr_height * 0.8)


def get_min_selected_plate_area(candidates: list[SelectedCandidate]) -> int:
    """Return the smallest original bbox area among selected detected candidates."""

    areas = [
        candidate.detection_area
        for candidate in candidates
        if candidate.detected and candidate.detection_area > 0
    ]
    if not areas:
        return 0
    return min(areas)
...
def should_use_sr(candidates: list[SelectedCandidate], options: PipelineOptions) -> bool:
    ...
    if sr_mode == "always":
        return True
    if sr_mode in {"skip", "never"}:
        return False

    return get_min_selected_plate_area(candidates) <= get_sr_area_threshold(options)
