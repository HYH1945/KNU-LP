...
def denoise_single(image: UploadedImage, model: nn.Module, cfg: dict) -> np.ndarray:
    ...
    Input:
        ...
        cfg   (dict)          : settings.yaml의 denoiser 섹션
    Output:
        denoised (np.ndarray) : 디노이즈된 RGB 이미지 (H, W, 3) -> BGR로 변환하여 리턴, uint8,
                                입력 해상도로 복원
    """
    # bytes → BGR ndarray
...
if __name__ == "__main__":
    """
    단독 실행 테스트:
    - settings.yaml 로드 → 모델 빌드 → 더미 컬러 이미지 1장 추론
    ...
    # 더미 컬러 BGR 이미지 생성 (YOLO crop 사이즈 335x170x3)
    ...
    assert result.shape == (170, 335, 3), f"출력 shape 오류: {result.shape}"
    assert result.dtype == np.uint8, f"출력 dtype 오류: {result.dtype}"
    print(f"[OK] denoise_single 단독 테스트 통과 | shape={result.shape}, dtype={result.dtype}")
